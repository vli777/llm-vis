...
export default function BoxChart({ spec }: { spec: ChartSpec }) {
  ...
  return (
    <div style={{ width: "100%", height: "100%", overflow: "auto" }}>
      ...
      >
        {/* Y axis */}
        <line x1={padding} y1={padding} x2={padding} y2={svgHeight - padding} stroke="#636EFA" />
        {yTicks.map((t, i) => (
          <g key={i}>
            <line x1={padding - 4} y1={scaleY(t)} x2={padding} y2={scaleY(t)} stroke="#636EFA" />
            <text x={padding - 8} y={scaleY(t) + 4} fill="#636EFA" fontSize={10} textAnchor="end">
              {t.toFixed(1)}
            </text>
          </g>
        ))}
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              {/* Whisker line */}
              <line x1={cx} y1={scaleY(d.max)} x2={cx} y2={scaleY(d.min)} stroke="#636EFA" strokeWidth={1} />
              {/* Whisker caps */}
              <line x1={x1 + 10} y1={scaleY(d.max)} x2={x2 - 10} y2={scaleY(d.max)} stroke="#636EFA" strokeWidth={1} />
              <line x1={x1 + 10} y1={scaleY(d.min)} x2={x2 - 10} y2={scaleY(d.min)} stroke="#636EFA" strokeWidth={1} />
              {/* Box */}
              <rect
                x={x1}
                y={scaleY(d.q3)}
              ...
              />
              {/* Median line */}
              <line x1={x1} y1={scaleY(d.median)} x2={x2} y2={scaleY(d.median)} stroke="#EF553B" strokeWidth={2} />
              {/* Outliers */}
              {(d.outliers || []).slice(0, 20).map((o: number, j: number) => (
                <circle key={j} cx={cx} cy={scaleY(o)} r={3} fill="#EF553B" opacity={0.7} />
              ))}
              {/* Label */}
              <text
                x={cx}
